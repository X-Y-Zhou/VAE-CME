...
train_sol = readdlm("Birth-Death/Control_topology/after20231205/train_data_ssa2.csv", ',')[2:end,:]
ρ_list = readdlm("Birth-Death/Control_topology/after20231205/p.csv", ',')[2:end,:]
...
function f1!(x,p1,p2,ϵ)
    ...
    return vcat(-sum(ρ_list)*x[1]+NN[1]*x[2],[sum(p_list[i-j]*x[j] for j in 1:i-1) - 
