...
ab_list = [[0.5,0.25],[0.5,0.5],[0.5,1],[0.25,0.1],[0.25,0.5],
                [0.25,1],[0.25,1.5],[0.1,0.1],[0.1,2],[0.1,3]]
...
@time for epoch in 1:epochs
    ...
    if mse<mse_min[1]
        df = DataFrame(p1 = p1)
        CSV.write("Birth-Death/Control_topology/after20231218/params_trained_Poisson.csv",df)
        mse_min[1] = mse
    end
...
function plot_distribution(set)
    ...
    plot!(0:N-1,train_sol[set],linewidth = 3,label="exact",title=join(["a,b=",ab_list[set]]),line=:dash)
end
plot_distribution(1)
