...
train_sol = vec(readdlm("Birth-Death/Control_topology/after20231221/ssa_tele.csv", ',')[2:2N+1,:])
sigma_on = 0.003
sigma_off = 0.004
rho_on = 0.3
rho_off = 0.0
...
df = CSV.read("Birth-Death/Control_topology/after20231221/params_trained_tele.csv",DataFrame)
...
@time for epoch in 1:epochs
    ...
    if mse<mse_min[1]
        ...
        CSV.write("Birth-Death/Control_topology/after20231221/params_trained_tele.csv",df)
        mse_min[1] = mse
    end
...
solution_P = solution[1:N]+solution[N+1:2*N]
train_sol_P = train_sol[1:N]+train_sol[N+1:2*N]
mse = Flux.mse(solution_P,train_sol_P)

plot(0:N-1,solution_P,linewidth = 3,label="NN-CME",xlabel = "# of products \n", ylabel = "\n Probability")
plot!(0:N-1,train_sol_P,linewidth = 3,label="exact",line=:dash)
