...
# Calculate the probabilities at 0~N for τ=120 and τ=30
end_time = 1200
...
function loss_func_120(p1,p2,ϵ)
    ...
    mse = Flux.mse(Array(sol_cme),train_sol_120[:,saveat.+1])
    print("mse:",mse," ")
...
function loss_func_30(p1,p2,ϵ)
    ...
    mse = Flux.mse(Array(sol_cme),train_sol_30[:,saveat.+1])
    print("mse:",mse," ")
...
params2
df = DataFrame(params1 = vcat(params1,[0 for i=1:length(params2)-length(params1)]),params2 =params2)
CSV.write("Bursty/Control_mean/params_ode_control_mean.csv",df)
...
# Predict probability distribution for τ=30~120
