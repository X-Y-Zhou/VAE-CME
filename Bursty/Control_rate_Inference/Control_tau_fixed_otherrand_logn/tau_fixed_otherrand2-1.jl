...
function plot_distribution_1(set)
    ...
    plot!(0:N-1,train_sol_1[:,set],linewidth = 3,label="exact",title=join(["a,b=",ab_list[set]," var = 8883129"]),line=:dash)
end

function plot_distribution_2(set)
    plot(0:N-1,solution_2[set],linewidth = 3,label="VAE-CME",xlabel = "# of products \n", ylabel = "\n Probability")
    plot!(0:N-1,train_sol_2[:,set],linewidth = 3,label="exact",title=join(["a,b=",ab_list[set]," var = 19045"]),line=:dash)
end
...
plot_all()
savefig("Control_rate_Inference/Control_tau_fixed_otherrand_logn/fitting.pdf")
...
plot_all()
savefig("Control_rate_Inference/Control_tau_fixed_otherrand_logn/pre_set$set.pdf")
