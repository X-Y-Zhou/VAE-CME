...
model = Chain(Dense(N, 10, tanh), Dense(10, N-1), x -> 0.03.*x .+ [i/120 for i in 1:N-1], x -> relu.(x));
...
@time for epoch in 1:epochs
    ...
    if mse<mse_min[1]
        df = DataFrame(p1 = p1)
        CSV.write("Bursty/Control_rate_Inference/control_kinetic/params_bp.csv",df)
        mse_min[1] = mse
    end
...
df = CSV.read("Bursty/Control_rate_Inference/control_kinetic/params_bp.csv",DataFrame)
p1 = df.p1
ps = Flux.params(p1)
...
function plot_distribution(set)
    plot(0:N-1,solution[set],linewidth = 3,label="MLP-CME",xlabel = "# of products \n", ylabel = "\n Probability")
