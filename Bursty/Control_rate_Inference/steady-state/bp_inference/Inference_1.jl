...
df = CSV.read("Bursty/Control_rate_Inferencesteady-state/bp_inference/params_bp_abcd4-1.csv",DataFrame)
p1 = df.p1
...
set = 1
dataset = 1
SSA_data = readdlm("Bursty/Control_rate_Inferencesteady-state/bp_inference/Inference_data/set$set/$dataset.csv",',')[2:end,:]

@time for dataset = 1:5
print(dataset,"\n")
SSA_data = readdlm("Bursty/Control_rate_Inferencesteady-state/bp_inference/Inference_data/set$set/$dataset.csv",',')[2:end,:]
# SSA_data = readdlm("Bursty/Control_rate_Inferencesteady-state/bp_inference/data/set$dataset.csv",',')[2:end,:]
...
function check_inference(kinetic_params)
    ...
    df = CSV.read("Bursty/Control_rate_InferenceControl_tau_fixed_otherrand_erlang/params_tfo2.csv",DataFrame)
    params1 = df.params1
...
SSA_data = vec(readdlm("Bursty/Control_rate_InferenceControl_tau_fixed_otherrand_erlang/data/set$set/$(width).csv",',')[2:end,:])
