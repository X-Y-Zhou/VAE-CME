...
df = CSV.read("params_bp_abcd4-1.csv",DataFrame)
p1 = df.p1
...
set = 5
dataset = 1
SSA_data = readdlm("Inference_data/set$set/$dataset.csv",',')[2:end,:]

@time for dataset = 1:5
print(dataset,"\n")
SSA_data = readdlm("Inference_data/set$set/$dataset.csv",',')[2:end,:]
# SSA_data = readdlm("data/set$dataset.csv",',')[2:end,:]
