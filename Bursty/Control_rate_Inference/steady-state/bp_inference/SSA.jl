...
df = DataFrame(reshape(train_sol_people[:,end],N+1,1),:auto)
CSV.write("Control_rate_Inference/steady-state/bp_inference/Inference_data/set$set/$epoch.csv",df)
# CSV.write("Control_rate_Inference/steady-state/bp_inference/data/set$set.csv",df)
end