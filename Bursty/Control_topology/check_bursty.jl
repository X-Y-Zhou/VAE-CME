...
# model 
model = Chain(Dense(N, 500, tanh), Dense(500, N-1), x->exp.(x));
...
@time for epoch in 1:epochs
    ...
    if mse<mse_min[1]
        df = DataFrame(p1 = p1)
        CSV.write("Control_topology/check_bursty.csv",df)
        mse_min[1] = mse
    end
    push!(mse_list,mse)
    print(mse,"\n")
...
mse_min = [0.01146657902512305]

using CSV,DataFrames
df = CSV.read("Control_topology/check_bursty.csv",DataFrame)
p1 = df.p1
ps = Flux.params(p1);
