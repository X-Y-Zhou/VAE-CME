...
SSA_histo = [counts(round.(Int, train_sol[:,saveat[i]].*traj))[1:8] for i=1:length(saveat)]
...
function loss_func(p1,p2,ϵ)
    ...
    log_value = [log.(temp[i]) for i=1:length(saveat)]
    logp_x_z = sum([sum(SSA_histo[i].*log_value[i][1:8])/traj for i=1:length(saveat)])/length(saveat)
    print(logp_x_z," ")
...
@time for epoch in 1:epochs
    ...
    print(Flux.mse(solution,exact_sol),"\n")
end
...
df = CSV.read("machine-learning/ode/bursty/Reduce_sample_size/params_VAE2_$traj.csv",DataFrame)
params1 = df.params1
...
CSV.write("machine-learning/ode/bursty/Reduce_sample_size/params_VAE2_$traj.csv",df)