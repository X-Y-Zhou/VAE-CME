...
df = CSV.read("Oscillation/params_origin_trained.csv",DataFrame)
...
CSV.write("Oscillation/params_origin_trained.csv",df)

function plot_distribution_X(time_choose)
    p=plot(0:N,sol_X[:,time_choose+1],label="X",linewidth = 3,xlabel = "# of products", ylabel = "\n Probability")
...
function plot_distribution_Y(time_choose)
    p=plot(0:N,sol_Y[:,time_choose+1],label="Y",linewidth = 3,xlabel = "# of products", ylabel = "\n Probability")
...
plot_mean()
savefig("Oscillation/xy_mean.pdf")
