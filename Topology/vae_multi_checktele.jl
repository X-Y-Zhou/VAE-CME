...
# bd params
ps_matrix = vec(readdlm("Topology/ps_bd2.csv"))
ρ_list = ps_matrix
...
sol_bd(p1,p2,ϵ,ρ,P0) = nlsolve(x->f1!(x,p1,p2,ϵ,ρ),P0).zero

function solve_bd(ρ,p1,p2,ϵ)
...
function f_tele!(x,p1,p2,ϵ,sigma_on,sigma_off,rho_on)
    # NN1 = re(p)(x[1:N])
    # NN2 = re(p)(x[N+1:2*N])
...
sol_tele(p1,p2,ϵ,sigma_on,sigma_off,rho_on,P_0) = nlsolve(x->f_tele!(x,p1,p2,ϵ,sigma_on,sigma_off,rho_on),P_0).zero

# P0,P1
function solve_tele(sigma_on,sigma_off,rho_on,p1,p2,ϵ)
...
df = CSV.read("Topology/params_trained_vae_tele3.csv",DataFrame)
p1 = df.p1
ps = Flux.params(p1);
# end
