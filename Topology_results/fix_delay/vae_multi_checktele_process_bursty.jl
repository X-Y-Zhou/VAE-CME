...
@everywhere ps_matrix_tele = readdlm("Topology_results/fix_delay/tele_data/ps_telev$version.txt")
...
check_sol = readdlm("Topology_results/fix_delay/tele_data/matrix_telev$version.txt")

# bursty params and train_sol
@everywhere ps_matrix_bursty = readdlm("Topology_results/fix_delay/ps_burstyv1.txt")
...
using CSV,DataFrames
df = CSV.read("Topology_results/fix_delay/params_trained_vae_tele_burstyv3_better.csv",DataFrame)
params1 = df.params1[1:length(params1)]
params2 = df.params2[1:length(params2)]
ps = Flux.params(params1,params2);
# end
...
for i = 1:15
    p = plot_channel(i)
    savefig(p,"Topology_results/fix_delay/topo_results/fig_$i.svg")
end
