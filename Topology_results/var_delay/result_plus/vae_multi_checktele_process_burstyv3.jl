...
@everywhere ps_matrix_tele = readdlm("Topology_results/var_delay/result_plus/ps_tele.txt")
...
check_sol1 = readdlm("Topology_results/var_delay/result_plus/tele_SSA_Attr=0.25.txt") # Attrtibute = 0
check_sol2 = readdlm("Topology_results/var_delay/result_plus/tele_SSA_Attr=0.75.txt") # Attrtibute = 1
check_sol3 = readdlm("Topology_results/var_delay/result_plus/tele_pre_Attr=0.50.txt") # Attrtibute = 0.5
# check_sol3 = check_sol3[:,[10,17,21,50]]
ps_matrix_tele
...
@everywhere ps_matrix_bursty = readdlm("Topology_results/var_delay/result_plus/ps_bursty.txt")
...
train_sol1 = readdlm("Topology_results/var_delay/result_plus/bursty_SSA_Attr=0.0.txt") # var = 0
train_sol2 = readdlm("Topology_results/var_delay/result_plus/bursty_SSA_Attr=1.0.txt") # var = max
train_sol3 = readdlm("Topology_results/var_delay/result_plus/bursty_SSA_Attr=0.5.txt") # var = 0
# train_sol = train_sol[:,[2,3,5,48]]
...
@everywhere function f3!(x,p1,p2,ϵ,a,b)
    h = re1(p1)(x)
    μ, logσ = split_encoder_result(h, latent_size)
    z = reparameterize.(μ, logσ, ϵ)
    z = vcat(z,0.5)
...
@everywhere sol_bursty3(p1,p2,ϵ,a,b,P0) = nlsolve(x->f3!(x,p1,p2,ϵ,a,b),P0).zero
...
@everywhere function solve_bursty3(a,b,p1,p2,ϵ)
    P_0_distribution = NegativeBinomial(a*τ, 1/(1+b))
    P_0 = [pdf(P_0_distribution,i) for i=0:N-1]
    solution = sol_bursty3(p1,p2,ϵ,a,b,P_0)
    return solution
end
...
@time solution_bursty1 = hcat(pmap(i->solve_bursty1(a_list[i],b_list[i],params1,params2,ϵ),1:batchsize_bursty)...);
@time solution_bursty2 = hcat(pmap(i->solve_bursty2(a_list[i],b_list[i],params1,params2,ϵ),1:batchsize_bursty)...);
@time solution_bursty3 = hcat(pmap(i->solve_bursty3(a_list[i],b_list[i],params1,params2,ϵ),1:batchsize_bursty)...);

mse_bursty1 = Flux.mse(solution_bursty1,train_sol1)
mse_bursty2 = Flux.mse(solution_bursty2,train_sol2)
mse_bursty3 = Flux.mse(solution_bursty3,train_sol3)
mse_bursty = mse_bursty1 + mse_bursty2
# mse_min = [mse_bursty]

ϵ = zeros(latent_size)
@time solution_bursty = hcat(pmap(i->solve_bursty1(a_list[i],b_list[i],params1,params2,ϵ),1:batchsize_bursty)...);
mse_bursty = Flux.mse(solution_bursty,train_sol)

writedlm("Topology_results/var_delay/result_plus/bursty_pre_Attr=0.0.txt",solution_bursty1)
writedlm("Topology_results/var_delay/result_plus/bursty_pre_Attr=1.0.txt",solution_bursty2)
writedlm("Topology_results/var_delay/result_plus/bursty_pre_Attr=0.5.txt",solution_bursty3)
...
writedlm("Topology_results/var_delay/result_plus/tele_pre_Attr=0.25.txt",solution_tele1)
writedlm("Topology_results/var_delay/result_plus/tele_pre_Attr=0.75.txt",solution_tele2)
writedlm("Topology_results/var_delay/result_plus/tele_pre_Attr=0.50.txt",solution_tele3)

writedlm("Topology_results/var_delay/result_plus/tele_SSA_Attr=0.50.txt",check_sol3)
...
function plot_distribution(set)
    ...
    plot!(0:N-1,train_sol[:,set],linewidth = 3,label="exact",title=join([round.(ps_matrix_bursty[:,set],digits=4)]),line=:dash)
end
