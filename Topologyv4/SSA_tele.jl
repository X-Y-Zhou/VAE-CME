...
ps_matrix = readdlm("Topologyv4/tele/data/ps_telev1.txt")
batchsize = size(ps_matrix,2)

# tele
dist = LogNormal(0,sqrt(4))+100
sigma_on,sigma_off,ρ = ps_matrix[:,40]
...
for j = 1:2
    T1 = T1T2_list[j][1]
    T2 = T1T2_list[j][2]
    matrix_tele = readdlm("Topologyv4/tele/data/matrix_tele_$T1-$T2.csv")
