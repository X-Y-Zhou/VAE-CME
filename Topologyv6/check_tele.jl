...
version = 1
ps_matrix_tele = readdlm("Topologyv6/tele/data/ps_telev$version.txt")
sigma_on_list = ps_matrix_tele[1,:]
sigma_off_list = ps_matrix_tele[2,:]
rho_on_list = ps_matrix_tele[3,:]
rho_off = 0.0
gamma= 0.0
batchsize_tele = size(ps_matrix_tele,2)
check_sol = readdlm("Topologyv6/tele/data/matrix_telev$version.csv") # var = 0 Attribute = 0

check_sol1 = readdlm("Topologyv6/tele/data/matrix_tele_100-100.csv") # var = 0 Attribute = 0
check_sol2 = readdlm("Topologyv6/tele/data/matrix_tele_0-200.csv") # var = max Attribute = 1
# check_sol3 = readdlm("Topologyv6/tele/data/matrix_tele_50-150.csv") # Attribute = 0.5


# bursty params and train_sol
ps_matrix_bursty = readdlm("Topologyv6/ps_burstyv1.csv")
batchsize_bursty = size(ps_matrix_bursty,2)
a_list = ps_matrix_bursty[1,:]
b_list = ps_matrix_bursty[2,:]

train_sol1 = readdlm("Topologyv6/bursty_data/matrix_bursty_100-100.csv") # var = 0 Attribute = 0
train_sol2 = readdlm("Topologyv6/bursty_data/matrix_bursty_0-200.csv") # var = max Attribute = 1

# model initialization
latent_size = 2;
encoder = Chain(Dense(N, 10,tanh),Dense(10, latent_size * 2));
decoder_1 = Chain(Dense(latent_size+1, 10),Dense(10 , N-1),x -> x.+[i/τ  for i in 1:N-1],x ->relu.(x));
...
sol_bursty1(p1,p2,ϵ,a,b,P0) = nlsolve(x->f1!(x,p1,p2,ϵ,a,b),P0).zero
sol_bursty2(p1,p2,ϵ,a,b,P0) = nlsolve(x->f2!(x,p1,p2,ϵ,a,b),P0).zero

function solve_bursty1(a,b,p1,p2,ϵ)
...
function solve_bursty2(a,b,p1,p2,ϵ)
    P_0_distribution = NegativeBinomial(a*τ, 1/(1+b))
    P_0 = [pdf(P_0_distribution,i) for i=0:N-1]
...
function f_tele!(x,p1,p2,ϵ,sigma_on,sigma_off,rho_on,Attribute)
    h = re1(p1)(x[1:N])
    μ, logσ = split_encoder_result(h, latent_size)
    z = reparameterize.(μ, logσ, ϵ)
    z = vcat(z,Attribute)
...
sol_tele(p1,p2,ϵ,sigma_on,sigma_off,rho_on,P_0,Attrtibute) = nlsolve(x->f_tele!(x,p1,p2,ϵ,sigma_on,sigma_off,rho_on,Attrtibute),P_0).zero

# P0,P1
function solve_tele(sigma_on,sigma_off,rho_on,p1,p2,ϵ,Attrtibute)
...
Attrtibute = 1
solution_tele = solve_tele(sigma_on,sigma_off,ρ,params1,params2,ϵ,Attrtibute)
plot!(0:N-1,solution_tele,lw=3)
